...
const AnnotationItem = ({
  ...
  const { incidents, name, label, color, parentName, childrenNames } = itemData;
  ...
  const warningText = isEmptyCheckEnable && incidents.length < 2 && (
    <span className="text-danger">{emptyCheckAnnotationItemWarningText}</span>
  );
    ...
    >
      <div className="d-flex align-items-center mb-2">
        ...
        <OpenDialogButton
          ...
          onDontShowAgainChange={e => {
            console.log('dispatch delete action', e.target.checked);
            dispatchIsDialogDisabled({
