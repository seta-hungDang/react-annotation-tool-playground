...
const IncidentList = ({ className, annotationName, incidents }) => {
  ...
  const incidentListUI = incidents.map((incident, index) => {
    const { name, time, width, height, status, x, y } = incident;
        ...
        >
          ...
          >
            ...
            <div className="incident-list__item-size pr-1">
              <b>{t('incidentSize')}</b> <RoundedNumber number={width} />
              {'x'}
              <RoundedNumber number={height} />
            </div>
            <div className="incident-list__item-position">
              <b>{t('incidentPosition')}</b> <RoundedNumber number={x} />
              {':'}
          ...
          <Button
            ...
            color="link"
            onClick={() =>
              onIncidentItemDeleteClick({ annotationName, incidentName: name })
            }
          >
            <MdDelete />
          </Button>
  ...
  return <ListGroup className={rootClassName}>{incidentListUI}</ListGroup>;
};
