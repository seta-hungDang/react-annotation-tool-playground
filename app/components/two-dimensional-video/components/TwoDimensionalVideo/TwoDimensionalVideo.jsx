...
class TwoDimensionalVideo extends Component {
  constructor(props) {
    ...
    if (defaultAnnotations && defaultAnnotations.length !== 0) {
      ...
      annotations.forEach(id => {
        entities.annotations[id].isManipulatable =
          props.isDefaultAnnotationsManipulatable;
      });
    }
    this.state = {
      isPreviewed:
        previewNoticeList && previewNoticeList.length === 0 && !previewHeader,
      isSubmitted: false,
      videoWidth,
  ...
  handleVideoReady = () => {
    this.setState({
      annotationHeight: document.getElementById('react-player').children[0]
        .clientHeight,
    });
  };
  ...
  handleCanvasStageMouseDown = e => {
    ...
    this.setState(
      prevState => {
        ...
        const { annotations, entities } = prevState;
        const incidents = [];
        incidents.push(
          Incident({
            id: `${uniqueKey}`,
  ...
  handleCanvasGroupDragEnd = e => {
    console.log('Group drag end', e.target.getClassName());
  ...
  handleCanvasDotMouseDown = e => {
    console.log('Canvas dot mouse down');
    const group = e.target.findAncestor('Group');
    this.setState({ focusing: group.name() });
  };

  handleCanvasDotDragEnd = e => {
    console.log('Dot drag end');
    ...
    const maxX = Math.max(
      topLeft.getAbsolutePosition().x,
      topRight.getAbsolutePosition().x,
      bottomRight.getAbsolutePosition().x,
      bottomLeft.getAbsolutePosition().x,
    );
    const minX = Math.min(
      topLeft.getAbsolutePosition().x,
      topRight.getAbsolutePosition().x,
    ...
    const maxY = Math.max(
      topLeft.getAbsolutePosition().y,
      topRight.getAbsolutePosition().y,
      bottomRight.getAbsolutePosition().y,
      bottomLeft.getAbsolutePosition().y,
    );
    const minY = Math.min(
      topLeft.getAbsolutePosition().y,
      topRight.getAbsolutePosition().y,
  ...
  handleIncidentItemDelete = e => {
    ...
    this.setState(prevState => {
      ...
      const { entities } = prevState;
      const { annotations } = entities;
      const incidents = entities.annotations[annotationName].incidents.filter(
        t => {
    ...
    this.setState(prevState => {
      ...
      if (!isNaN(label)) {
        ...
        if (`${lastLabel}` !== '1' && `${lastLabel}` !== label) {
          const lastName = annotations.find(
            a => entitiesAnnotations[a].label === `${lastLabel}`,
          );
      ...
      if (entitiesAnnotations[name].parentName) {
        const parent =
          entitiesAnnotations[entitiesAnnotations[name].parentName];
        const i = parent.childrenNames.indexOf(name);
        if (i !== -1) {
          parent.childrenNames.splice(i, 1);
          if (
            parent.childrenNames.length == 0 &&
            parent.incidents[parent.incidents.length - 1].status === SPLIT
          )
            parent.incidents[parent.incidents.length - 1].status = SHOW;
        }
      }
      ...
      return {
        annotations,
        entities: { ...entities, annotations: entitiesAnnotations },
        focusing: '',
      };
    });
  };
  ...
  handleListAnnotationShowHide = e => {
    ...
    this.setState(prevState => {
      ...
      if (status === HIDE)
        entities.annotations[name].clearRedundantIncidents(status);
  ...
  renderAddButtonUI = () => {
    ...
    const isAddButtonAvailable =
      defaultNumRootAnnotations + numAnnotationsCanBeAdded >
      getLastAnnotationLabel(annotations, entities);
  ...
  render() {
    ...
    const rootClassName = `two-dimensional-video${
      className ? ` ${className}` : ''
    }`;
    return (
